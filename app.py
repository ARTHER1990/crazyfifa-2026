# Last cache clear and score update: 2026-07-20 08:22 (Auto updated: Brazil 1-2 Norway)
...
if menu == "ศึกชิงแชมป์โลก 2026 (World Cup)":
    ...
    try:
        ...
        if not leaderboard_df.empty:
            ...
            # เรนเดอร์กล่องแสดงความยินดีกับแชมป์เปี้ยนและโพเดียม 3 มิติระดับพรีเมียม
            st.markdown(
                """
                <div class="champion-wrap">
                    <style>
                        .champion-wrap {
                            font-family: 'Kanit', sans-serif;
                            background: linear-gradient(135deg, rgba(25, 20, 40, 0.8) 0%, rgba(10, 10, 20, 0.95) 100%);
                            border: 2px solid rgba(255, 215, 0, 0.4);
                            border-radius: 20px;
                            padding: 35px 20px;
                            text-align: center;
                            box-shadow: 0 0 35px rgba(255, 215, 0, 0.15), inset 0 0 20px rgba(255, 215, 0, 0.05);
            ...
            # รันสคริปต์พ่นกระดาษสี Confetti ไปยัง Document หลักของ Streamlit
            confetti_script = """
            <script>
                function launchConfetti() {
                    const parentDoc = window.parent.document;
                    if (!parentDoc.getElementById('confetti-script-cdn')) {
                        const script = parentDoc.createElement('script');
                        script.id = 'confetti-script-cdn';
                        script.src = 'https://cdn.jsdelivr.net/npm/canvas-confetti@1.6.0/dist/confetti.browser.min.js';
                        script.onload = () => {
                            triggerConfetti(parentDoc);
                        };
                ...
                function triggerConfetti(doc) {
                    if (typeof doc.defaultView.confetti === 'function') {
                        doc.defaultView.confetti({
                            particleCount: 160,
                            spread: 90,
                            origin: { y: 0.55 }
                        });
                        
                        // เสริมความยิ่งใหญ่ด้วยการยิงสองข้างเฉียงขึ้น
                        setTimeout(() => {
                            doc.defaultView.confetti({
                                particleCount: 80,
                                angle: 60,
            ...
            """
            components.html(confetti_script, height=0)
    except Exception as e:
        pass
