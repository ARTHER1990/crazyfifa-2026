...
def main():
    print("🔒 เริ่มกระบวนการปิดรับทายผลสำหรับ แมตช์ ID 81 (United States vs Bosnia and Herzegovina) ทันที...")

    target_id = '81'
    # ตั้งเวลาในอดีต (ก่อนเวลาระบบปัจจุบัน 07:35) เพื่อปิดรับทายผลทันทีบนหน้าเว็บ
    locked_time = '2026-07-02 07:30:00' 
    ...
    try:
        # กระตุ้นให้ล้างแคช Streamlit
        app_path = os.path.join(BASE_DIR, 'app.py')
        if os.path.exists(app_path):
            with open(app_path, 'r', encoding='utf-8') as f:
                content = f.read()
            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
            comment_line = f'# Last cache clear: {now_str} (Manual locked Match 81)\n'
            ...
            with open(app_path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print("✅ ปรับแต่งหัวไฟล์ app.py สำเร็จ เพื่อบังคับล้างแคช RAM บน Cloud")
            
        print("✅ ดำเนินการล็อกการทายผลสำเร็จสมบูรณ์!")
    except Exception as e:
        print(f"❌ เกิดข้อผิดพลาด: {e}")

    print("\n🏆 --- การปิดรับทายผล Match 81 เสร็จสิ้น --- 🏆")
